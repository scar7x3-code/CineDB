...
function renderDetail(m) {
  document.title = `${escapeHtml(m.Title)} (${escapeHtml(m.Year)}) — CineDB`;
  ...
  const metaPills = [
    ...
    .filter(Boolean)
    .map(p => `<span class="meta-pill">${escapeHtml(p)}</span>`)
    .join('');

  const ratingsHtml = hasRatings
    ? m.Ratings.map(r => `
        <div class="score-row">
          <span class="score-source">${escapeHtml(ratingSource(r.Source))}</span>
          <span class="score-value">${escapeHtml(r.Value)}</span>
        </div>
      `).join('')
    : '';

  const genreHtml = hasGenre
    ? hasGenre.split(',').map(g => `<span class="genre-tag">${escapeHtml(g.trim())}</span>`).join('')
    : '';

  const titleEsc = escapeHtml(m.Title);
  const typeEsc  = escapeHtml(m.Type);
  const writerLine = (sanitize(m.Writer.split(',')[0].trim()) || m.Writer);
  const writerEsc  = escapeHtml(writerLine);
  ...
  detailContent.innerHTML = `
    ...
    <div class="detail-layout">
      ...
      <div class="detail-poster-wrap">
        ${hasPoster
          ? `<img class="detail-poster" src="${escapeHtml(hasPoster)}" alt="${titleEsc}" />`
      ...
      <div class="detail-info">
        <span class="detail-type-badge">${typeEsc}</span>
        <h1 class="detail-title">${titleEsc}</h1>
        ${m.Awards && m.Awards !== 'N/A' ? `<p class="detail-tagline">${escapeHtml(m.Awards)}</p>` : ''}
        ...
        ${hasPlot ? `
          ...
          <p class="detail-plot">${escapeHtml(hasPlot)}</p>
          <hr class="detail-divider" />
        ` : ''}
        ...
        <div class="detail-credits">
          ${hasDirector ? `
            <div class="credit-group">
              ...
              <p class="credit-value">${escapeHtml(hasDirector)}</p>
            </div>
          ` : ''}
          ${hasWriter ? `
            <div class="credit-group">
              <p class="section-label">Writer</p>
              <p class="credit-value">${writerEsc}</p>
            </div>
          ` : ''}
          ${hasActors ? `
            <div class="credit-group" style="grid-column: 1 / -1">
              <p class="section-label">Cast</p>
              <p class="credit-value">${escapeHtml(hasActors)}</p>
            </div>
          ` : ''}
          ${m.Language && m.Language !== 'N/A' ? `
            <div class="credit-group">
              <p class="section-label">Language</p>
              <p class="credit-value">${escapeHtml(m.Language.split(',')[0].trim())}</p>
            </div>
          ` : ''}
          ${m.BoxOffice && m.BoxOffice !== 'N/A' ? `
            <div class="credit-group">
              <p class="section-label">Box Office</p>
              <p class="credit-value">${escapeHtml(m.BoxOffice)}</p>
            </div>
          ` : ''}
        </div>
